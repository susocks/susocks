...
func NewWSPool(url2 *url.URL) *WSPool {
	...
	for i := 0; i < *worker; i++ {
		go wsPool.NewConn(url2, strconv.FormatInt(connPoolID, 10))
	}
	go func() {
		for {
			data := <-wsPool.WSReadChan
			go func() {
				wsPool.mutex.Lock()
...
func (wsPoll *WSPool) NewConn(u *url.URL, connPoolID string) (*websocket.Conn, error) {
	...
	if err != nil {
		...
		go wsPoll.NewConn(u, connPoolID)
		return nil, err
	}
...
func ProcessUserConn(wsPool *WSPool, userConn net.Conn) {
	...
	ctx, cancelFunc := context.WithCancel(context.Background())
	csChan := make(chan []byte, 100)
	scChan := make(chan model.Pack, 100)
			...
			case data := <-scChan:
				...
				for {
					...
					if ok {
						...
						if len(WSReadWaitMap) > 0 { // TODO: clean && shutdown
